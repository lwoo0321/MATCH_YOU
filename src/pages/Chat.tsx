    ...
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="space-y-1 px-4 pt-3">
        {mockChatRooms.map((room) => (
          <Card
            key={room.id}
            className="cursor-pointer border-none shadow-sm transition-colors hover:bg-secondary/50"
        ...
        <Button variant="outline" className="mt-4 w-full" onClick={() => navigate('/chat/archive')}>
          <Bookmark className="mr-2 h-4 w-4" /> Saved Messages
        </Button>
      </div>
    </div>
  );
              ...
              <div className={`group max-w-[80%] ${isMe ? 'items-end' : 'items-start'}`}>
                ...
                >
                  ...
                  <button
                    ...
                    className={`absolute -bottom-1 ${isMe ? '-left-6' : '-right-6'} opacity-0 group-hover:opacity-100 transition-opacity`}
                  >
                ...
                <p className="mt-1 text-[10px] text-muted-foreground">
                  ...
                  {msg.saved && <span className="ml-1">• Saved</span>}
                </p>
              </div>
            </div>
          );
...
const ChatArchive = () => {
  const navigate = useNavigate();
  const allSaved = Object.values(mockMessages)
    .flat()
    .filter((m) => m.saved);
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      <div className="safe-top sticky top-0 z-10 flex items-center gap-3 bg-card px-4 pb-3 pt-3 shadow-sm">
        ...
        <h2 className="font-semibold">Saved Messages</h2>
      </div>
      <div className="space-y-2 px-4 pt-3">
        {allSaved.length === 0 ? (
          <p className="py-8 text-center text-sm text-muted-foreground">No saved messages yet. Long-press or hover over a message to save it.</p>
        ) : (
          allSaved.map((msg) => (
            <Card
              key={msg.id}
            ...
            >
              <CardContent className="p-4">
                ...
                <div className="mt-1 flex items-center gap-2">
                  <span className="text-[10px] text-muted-foreground">{format(new Date(msg.timestamp), 'MMM d, HH:mm')}</span>
                  {msg.labels?.map((l) => (
                    <span key={l} className="rounded-full bg-secondary px-2 py-0.5 text-[10px]">{l}</span>
                  ))}
                </div>
              </CardContent>
            </Card>
